...
def test_session_headers_sent_from_active_thread(mock_opencode_client):
    from msgflux.models.providers.opencode import (
        OpenCodeChatCompletion,
        OpenCodeGoChatCompletion,
    )
    ...
    tier, client = mock_opencode_client
    cls = OpenCodeChatCompletion if tier == "zen" else OpenCodeGoChatCompletion
    _chat_completion_ok(client)
    _responses_ok(client)

    with thread_context(thread_id="thread_1"):
        cls(model_id="kimi-k3", api_mode="chat_completions")("Hello")
        chat_headers = client.return_value.chat.completions.create.call_args.kwargs[
            "extra_headers"
        ]
        cls(model_id="kimi-k3", api_mode="responses")("Hello")
        responses_headers = client.return_value.responses.create.call_args.kwargs[
            "extra_headers"
        ]

    expected = {"User-Agent": "msgflux", "x-opencode-session": "thread_1"}
    assert chat_headers == expected
    assert responses_headers == expected


def test_no_session_header_without_active_thread(mock_opencode_client):
    from msgflux.models.providers.opencode import (
        OpenCodeChatCompletion,
        OpenCodeGoChatCompletion,
    )
    ...
    cls(model_id="kimi-k3", api_mode="chat_completions")("Hello")
    headers = client.return_value.chat.completions.create.call_args.kwargs[
        "extra_headers"
    ]
