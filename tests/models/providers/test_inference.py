"""Tests for the Inference.net OpenAI-compatible provider."""
...
def mock_inference_client():
    from msgflux.models.providers.inference import InferenceChatCompletion

    client = MagicMock()
    async_client = MagicMock()
    transport = EndpointMockTransport(client.return_value, async_client.return_value)
    with patch.object(InferenceChatCompletion, "chat_transport", transport):
        yield client
...
def test_inference_defaults_to_chat_completions():
    ...
    model = InferenceChatCompletion(model_id="glm-5.2")

    assert model.provider == "inference"
...
def test_inference_missing_api_key_raises(monkeypatch):
    ...
    with pytest.raises(ValueError, match="INFERENCE_API_KEY"):
        InferenceChatCompletion(model_id="glm-5.2")


def test_inference_models_registered():
    from msgflux.models.registry import model_registry

    assert "inference" in model_registry.get("chat_completion", {})
