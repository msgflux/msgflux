...
| `input_tokens_details.cached_tokens` | Input tokens read from a provider cache. `None` when the provider does not report it; a value greater than zero confirms a cache hit. |
| `cache_hit_percentage` | Percentage of input tokens served from the provider cache (`cached_tokens / input_tokens * 100`). `None` when the provider does not report cached tokens or a valid input-token denominator; `0.0` only when the provider explicitly reports zero cached tokens. |
...
Missing optional detail counters are `None` (unknown), while explicit provider
zeros stay `0`; unavailable provider cost is `None`. Top-level
input/output/total token counts and durations still default to `0` when the
provider omits them.
